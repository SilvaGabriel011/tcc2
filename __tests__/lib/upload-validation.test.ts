...
describe('upload-validation', () => {
  ...
  describe('scanFileForThreats', () => {
    ...
    it('should reject files with script tags', async () => {
      const file = new File(['<script>alert("xss")</script>'], 'malicious.csv', {
        type: 'text/csv',
      })
      const result = await scanFileForThreats(file)
    ...
    it('should reject files with iframe tags', async () => {
      const file = new File(['<iframe src="evil.com"></iframe>'], 'malicious.csv', {
        type: 'text/csv',
      })
      const result = await scanFileForThreats(file)
    ...
    it('should reject files with Excel formula injection', async () => {
      const file = new File(['=IMPORTXML("http://evil.com")'], 'malicious.csv', {
        type: 'text/csv',
      })
      const result = await scanFileForThreats(file)
    ...
    it('should reject files with WEBSERVICE formula', async () => {
      const file = new File(['=WEBSERVICE("http://evil.com")'], 'malicious.csv', {
        type: 'text/csv',
      })
      const result = await scanFileForThreats(file)
