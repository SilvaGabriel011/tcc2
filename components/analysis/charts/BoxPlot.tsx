    ...
    <div className="w-full">
      ...
      <ResponsiveContainer width="100%" height={height}>
        <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <div className="space-y-1 text-sm">
                    <p className="text-muted-foreground">
                      ...
                      <span className="font-medium text-foreground">
                        {formatNumber(dataPoint.max, 2)}
                      </span>
                    </p>
                    <p className="text-muted-foreground">
                      Q3 (75%):{' '}
                      <span className="font-medium text-foreground">
                        {formatNumber(dataPoint.q3, 2)}
                      </span>
                    </p>
                    <p className="text-muted-foreground">
                      Mediana:{' '}
                      <span className="font-semibold text-foreground">
                        {formatNumber(dataPoint.median, 2)}
                      </span>
                    </p>
                    <p className="text-muted-foreground">
                      Q1 (25%):{' '}
                      <span className="font-medium text-foreground">
                        {formatNumber(dataPoint.q1, 2)}
                      </span>
                    </p>
                    <p className="text-muted-foreground">
                      Mínimo:{' '}
                      <span className="font-medium text-foreground">
                        {formatNumber(dataPoint.min, 2)}
                      </span>
                    </p>
                    {dataPoint.mean && (
          ...
          {/* Median line */}
          <ReferenceLine y={0} stroke="#ef4444" strokeWidth={2} label="Mediana" />
        </ComposedChart>
      </ResponsiveContainer>

      <div className="mt-4 text-sm text-muted-foreground">
        <p>
          <strong className="text-foreground">Box Plot:</strong> A caixa representa 50% dos dados
          (Q1 a Q3). A linha dentro da caixa é a mediana. Os whiskers mostram o alcance dos dados
          (excluindo outliers).
        </p>
      </div>
    </div>
