 ...
 */
export function pairedTTest(before: number[], after: number[], alpha: number = 0.05): TTestResult {
 ...
 * This is the proper implementation replacing the previous stub
 */
function incompleteBeta(a: number, b: number, x: number): number {
  ...
  if (useDirect) {
    ...
    // 1 - I_{1-x}(b, a)
    const front = Math.exp(b * Math.log(1 - x) + a * Math.log(x) - lnBeta - Math.log(b))
